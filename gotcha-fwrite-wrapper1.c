...
int fwrite1_init(int priority) {
    ...
    result = gotcha_set_priority("fwrite-wrapper1", priority);
    if (result != GOTCHA_SUCCESS) {
      printf("Error: fwrite-wrapper1 gotcha_set_priority returned %d\n", (int) result);
      return -1;
    } 
    result = gotcha_wrap(fwrite_wrap_actions, sizeof(fwrite_wrap_actions)/sizeof(struct gotcha_binding_t), "fwrite-wrapper1");
    if (result != GOTCHA_SUCCESS) {
      printf("Error: fwrite-wrapper1 gotcha_wrap returned %d\n", (int) result);
      return -1;
    }
    return 0;
}
